 ...
 */
 ...
 FdEntity::encrypt()    LINE 1085    which encrypts the files

 FdEntity::storekey()   LINE 961     which reads the text key pass and store it in variable

 MODIFIED FUNCTIONS:

 FdEntity::Read()       LINE 1007    added the decryption RC4 Functions
 FdEntity::Write()      LINE 1069    called the encrypt() function and stored them into bytes
...
unsigned char key_pass[100];
RC4_KEY myKey;
unsigned char bufCipher[4096];
...
//this function stores the key pass inside the variable key_pass

void FdEntity::storekey()
{
  ifstream inputFile(".passkey.txt");
  string firstLine;

  getline(inputFile, firstLine);

  strcpy( (char*) key_pass, firstLine.c_str() );
...
ssize_t FdEntity::Read(char* bytes, off_t start, size_t size, bool force_load)
{
  ...
  int ofst = 0;      //the offset that will increment with every loop to complete the image size
  int mod;           
  int theResult;
  char *theNewBytes;     //storing the new bytes

  theResult = rsize / 4096; //Computing how many times to run the loop


  for(int i= theResult; i>0; i--)
  {
    theNewBytes = new char[4096];
    memcpy(theNewBytes, bytes + ofst, 4096);
    RC4_set_key(&myKey, strlen((char *)key_pass) ,key_pass);

    RC4(&myKey, 4096,(unsigned char *) theNewBytes, (unsigned char *)bytes + ofst);

    ofst += 4096;

  }

  mod = rsize % 4096;          //computing if modulus != 0, that means there is remaining to take care of in image


  if(mod > 0) {
    theNewBytes = new char[mod];
    memcpy(theNewBytes, bytes + ofst, mod);

    RC4_set_key(&myKey, (unsigned)strlen((char*)key_pass), key_pass); 
    RC4 (&myKey, (unsigned)4096, (unsigned char*)theNewBytes, (unsigned char*)bytes + ofst);
...
ssize_t FdEntity::Write(const char* bytes, off_t start, size_t size)
{
  ...
  // Writing
  {
    ...
    storekey();   //call the function to store the key
...
const char* FdEntity::encrypt(const char* bytes, size_t size)
{

    RC4_set_key(&myKey, strlen((char *)key_pass) ,key_pass);
    RC4(&myKey, size,(unsigned char *) bytes, (unsigned char *)bufCipher);

    const char * x = ( const char *) bufCipher;
